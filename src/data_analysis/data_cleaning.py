...
def build_distribution_summary(datasets: dict, target="def"):
    """Build observation, default count, and default-rate summary by dataset."""
...
def build_bounds_table(df, lower=0.01, upper=0.99):
    """Build lower and upper quantile bounds for each numerical variable."""
...
def apply_iqr_bounds(train, test, oot, variables):
    """Clip selected variables with IQR bounds computed on the train dataset."""
...
def impute_missing_values(train, test, oot,
    ...
    # Compute statistics on the train dataset.
